...
--
-- Ordem das colunas (igual ao padrão de disciplinas: pai, familia_id, name, ordem, ativo):
...
--
-- familia_id é preenchido a partir de taxonomia.disciplinas (não precisa da coluna já existir).
-- Preserva dados, PK, FKs e grants típicos do Supabase.
-- Roda em uma única transação.
...
begin
  ...
  if exists (select 1 from taxonomia.assuntos_new where familia_id is null)
     or exists (select 1 from taxonomia.subassuntos_new where familia_id is null) then
    raise exception 'familia_id nulo após a cópia; confira disciplinas.familia_id';
  end if;
end $$;
